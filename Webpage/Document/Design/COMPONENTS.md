...
| 主标题 | `.wordmark` | pulse + pilot 双色，96 / 72 / 52 / 40 四档 |
| 口号 | `.slogan` | 脉搏所指，航向所至。 |
| 定位句 | `.position .blank` | 社会创新场景定位（长者居家照护）＋项目一句话简介，已回填 |
...
| 痛点表 | `.pain-table .pain-row`（`.k` / `.v`） | 现状问题（已回填，锚点跳 `#problems`）/ 影响范围 / 现有方案不足（待回填） |
...
- 断点：1024px（卡片 2 列、统计 2×2）、768px（导航 56px、隐藏导航链接）、640px（全单列）。
...
- 题目回填进度：`.ph-window` 与 `.feat-slot` 已填入正式内容；现状问题已回填（`#problems` 章节），痛点表剩余两行待数据核实后回填。
