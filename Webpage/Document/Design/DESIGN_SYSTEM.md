...
| 元素 | 字級 | 字重 | 字距 | 備註 |
|------|------|------|------|------|
| Hero wordmark（`pulsepilot`） | 96px | 900 | -2px | 響應式 72/52/40px |
...
1. **雙色 wordmark**：`pulse`（墨色）+ `pilot`（信號紅），字重 900、負字距。導航、Hero、頁腳三處一致。
...
| 痛點表格 | `.pain-table` | 200px 鍵列（紙底）+ mono 斜體值列；行 hover 變紙底 |
...
| `≤768px` | 平板 | wordmark 52px、導航 56px、網格單列、隱藏導航鏈接、痛點表上下堆疊 |
...
| 項目介紹 | `project.html` | 02 / 02·B | 項目窗口 ×2、核心功能 ×3、痛點表格 ×3；現狀問題章節 ×2（SVG 圖表 ×2 + 橫向條形圖 ×1）+ 資料來源 |
...
*最後更新：2026-08-02（project.html 加入現狀問題章節與數據圖表）*
