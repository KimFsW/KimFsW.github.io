...
| 元素 | 字級 | 字重 | 字距 | 備註 |
|------|------|------|------|------|
| Hero wordmark（`pulsepilot`＋`.w-sub`「（守望 AI）」） | 96px | 900 | -2px | 響應式 72/52/40px；後綴 26/22/18/15px 灰墨、基線對齊 |
...
1. **雙色 wordmark**：`pulse`（墨色）+ `pilot`（信號紅），字重 900、負字距，導航、Hero、頁腳三處一致；Hero 額外加 `.w-sub`「（守望 AI）」灰墨次級標註（26px，基線對齊）。
...
| `≤768px` | 平板 | wordmark 52px、導航 56px、網格單列、隱藏導航鏈接、不足清單單欄 |
